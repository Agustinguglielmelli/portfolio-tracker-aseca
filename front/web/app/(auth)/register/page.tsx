...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (form.password !== form.confirm) {
      return // La UI ya lo previene, pero por seguridad
    }
    ...
    try {
      ...
      if (!res.ok) {
        // Muestra el error que viene del backend (array de strings o string simple)
        const errorMessage = Array.isArray(data.message) ? data.message[0] : data.message
        setError(errorMessage || "Ocurrió un error en el registro")
        return
      }

      // Almacena el token localmente (US 1.3/1.4)
      ...
      <div className="size-full min-h-screen bg-[#050a1a] text-slate-200 relative overflow-hidden flex items-center justify-center px-4">
        ...
        <div className="relative w-full max-w-md">
          ...
          <div className="relative rounded-3xl bg-gradient-to-br from-[#0f1a35]/95 to-[#0a1224]/95 border border-white/10 shadow-[0_0_24px_rgba(56,189,248,0.1)] p-8 backdrop-blur-xl">
            ...
            {error && (
                <div className="mb-4 p-3 rounded-lg bg-rose-500/10 border border-rose-500/20 flex items-start gap-2 text-rose-300 text-sm">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                {passwordMismatch && (
                    <div className="text-xs text-rose-300 mt-1">{"Las contraseñas no son iguales, por favor, vuelva a ingresarlas."}</div>
                )}
              </div>
              ...
              >
                {isLoading ? "Creating..." : "Create account"} <ArrowRight className="w-4 h-4 ml-2" />
              </Button>
            </form>
